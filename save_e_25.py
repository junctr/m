...
# alpha_sa1_1 = 3.0
# alpha_sb1_1 = 3.0
# dir_base_1 = "./data/s_no/"


# alpha_lambda_0 = 0.0
# alpha_sa1_0 = 3.0
# alpha_sb1_0 = 3.0
# dir_base_0 = "./data/s_bz/"
# alpha_lambda_1 = 0.0
# alpha_sa1_1 = 3.0
# alpha_sb1_1 = 3.0
# dir_base_1 = "./data/s_no/"
...
for i in range(3):
    ...
    plt.plot(t_data, e_all_c[21+i],color="green",label="conventional")
    plt.plot(t_data, e_all_p[21+i],color="red",label="proposed")

    plt.xlabel("Time (s)")
    # plt.ylabel("norm($\it{e}$) (rad)")
    # plt.ylabel("$\it{||e||}$ (rad)")
    # plt.ylabel("$\it{|e_i|}$ (rad)")
    # plt.ylabel("$\it{|e_{%i}|}$ (rad)"%(i+1))
    # plt.ylabel("$\it{e_{%i}}$ (rad)"%(i+1))
    # plt.ylabel(r"$\it{\tau_{s%i}}$ (Nm)"%(i+1))
    # plt.ylabel("$\it{y_{%i}}$ (Nm)"%(i+1))
    plt.ylabel("$\it{d_{%i}}$ (Nm)"%(i+1))
    ...
    plt.savefig(f"fig/s_{i+1}.png")
    # plt.savefig(f"fig/m_{i+1}.png")
    # plt.savefig(f"fig/bz_{i+1}.png")
    # plt.savefig(f"fig/zd_{i+1}.png")
    # plt.savefig(f"fig/a_{i+1}.png")
