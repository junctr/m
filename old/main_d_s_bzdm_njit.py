...
# def main(n_seed):
    ...
    for i in range(int(end/step)):
        ...
        W += step * (alpha_w @ (mu.copy().reshape(5,1) - vk.T @ v.T.copy().reshape(-1,1) - ak.T @ a.T.copy().reshape(-1,1) - bk.T @ b.T.copy().reshape(-1,1)) @ s.T) + alpha_lambda * (Wold[-1] - Wold[-2])
        v += step * ((alpha_v @ vk @ W @ s).copy().reshape(5,15).T) + alpha_lambda * (vold[-1] - vold[-2])
        a += step * ((alpha_a @ ak @ W @ s).copy().reshape(5,15).T) + alpha_lambda * (aold[-1] - aold[-2])
        b += step * ((alpha_b @ bk @ W @ s).copy().reshape(5,15).T) + alpha_lambda * (bold[-1] - bold[-2])
        beta += step * k_beta
        zeta += step * k_zeta
        # D += step * k_D
