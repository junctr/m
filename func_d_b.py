...
def system(t, q, tau, p, l, g):
    ...
    dq[:,0:1] = q[:,1:2]
    dq[:,1:2] = np.linalg.inv(M(q,p,l)) @ (tau - tau0_f(t) - C(q,p,l) @ q[:,1:2] - G(q,p,l,g) - F(q))
    # dq[:,1:2] = np.linalg.inv(np.dot(M(q,p,l), tau - tau0_f(t) - np.dot(C(q,p,l), q[:,1:2]) - G(q,p,l,g) - F(q)))
...
def s_f(e):
    ...
    # s = e[:,1:2] + np.dot(5 * np.identity(3), e[:,0:1])
...
def y_f(muji, W):
    ...
    # y = np.dot(W.T, mu_f(muji).copy().reshape(5,1))
...
def taus0_f(s, beta, zeta, omega):
    ...
    # taus0 = (np.dot(beta.T, omega)**2 / (np.linalg.norm(s) * np.dot(beta.T, omega) + zeta)) * s

    return taus0

@njit(cache=True)
def taud_f(e, s, taus0, y):
